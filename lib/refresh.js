...
/**
 * Route the browser through a residential proxy when configured, so Cloudflare
 * on ads.openai.com sees a residential IP instead of Railway's datacenter IP
 * (the datacenter IP is what's tripping the bot-challenge). Two ways to set it:
 *   1. A single full URL with embedded creds — PROXY_URL or BRIGHTDATA_PROXY_URL,
 *      e.g. "http://brd-customer-XXX-zone-YYY:PASS@brd.superproxy.io:33335"
 *      (this is what competitor-dashboard already uses — reused here).
 *   2. Split parts — PROXY_SERVER (+ PROXY_USERNAME / PROXY_PASSWORD).
 * Returns undefined when none is set — Playwright then uses no proxy.
 */
function proxyConfig() {
  const full = process.env.PROXY_URL || process.env.BRIGHTDATA_PROXY_URL;
  if (full) {
    try {
      ...
      if (u.username) {
        ...
        // The reused Bright Data zone (competitor_monitor) is ROTATING residential.
        // A logged-in OpenAI session that hops IPs mid-run triggers "Something went
        // wrong", so pin to one US residential IP for the whole run via Bright Data's
        // sticky-session params (-country-us -session-<id>). Appended only if the
        // username doesn't already specify them.
        if (/-zone-/.test(user)) {
          if (!/-country-/.test(user)) user += '-country-us';
          if (!/-session-/.test(user)) user += `-session-${Math.random().toString(36).slice(2, 12)}`;
        }
        cfg.username = user;
      }
      ...
      console.error('[refresh] could not parse proxy URL:', e.message);
    }
  }
...
async function runRefresh({ trigger = 'manual', force = false } = {}) {
  ...
  let browser = null;
  try {
    browser = await chromium.launch({ headless: true, proxy: proxyConfig(), args: ['--no-sandbox', '--disable-setuid-sandbox', '--disable-blink-features=AutomationControlled'] });
    const context = await browser.newContext({
      storageState: session,
      userAgent: USER_AGENT,
      viewport: { width: 1440, height: 900 },
      ignoreHTTPSErrors: !!proxyConfig(), // Bright Data unlocker MITMs TLS
    });
...
async function diagnoseAudiences() {
  ...
  try {
    ...
    return { url: page.url(), login, cfCleared, proxied: !!pc, proxyServer: pc ? pc.server : null, ...diag };
  } finally {
    if (browser) await browser.close().catch(() => {});
  }
}
