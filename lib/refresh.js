...
const { getSession, fetchHashes, recordRun } = require('./storage');
...
async function createAudience(page, name, description, hashText) {
  await page.goto(AUDIENCES_URL, { waitUntil: 'networkidle', timeout: 60000 });
...
/** Read {status, matched, invalid} for the audience row matching `name`. */
...
async function waitForReady(page, name) {
  ...
  for (;;) {
    ...
    if (!row) throw new Error(`audience row "${name}" not found while polling`);
...
async function listCampaignNames(page) {
  await page.goto(CAMPAIGNS_URL, { waitUntil: 'networkidle', timeout: 60000 });
  if (await isLoginPage(page)) throw Object.assign(new Error('session expired'), { code: 'expired' });
...
async function archiveOldAudiences(page, newName) {
  await page.goto(AUDIENCES_URL, { waitUntil: 'networkidle', timeout: 60000 });
  await page.waitForTimeout(1500);
  const oldNames = await page.evaluate(({ base, keep }) => {
    const rows = [...document.querySelectorAll('tr,[role="row"]')];
    const names = [];
    for (const r of rows) {
      const t = r.textContent;
      if (t.includes(base) && !t.includes(keep)) {
        // reconstruct the display name: base plus optional date suffix right after it
        const m = t.match(new RegExp(`${base.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}( \\d{4}-\\d{2}-\\d{2})?`));
        if (m) names.push(m[0]);
      }
    }
    return [...new Set(names)];
  }, { base: BASE_NAME, keep: newName });

  let archived = 0;
  for (const nm of oldNames) {
    try {
      await rowMenuAction(page, nm, 'Archive');
      archived++;
      console.log(`[refresh] archived old audience "${nm}"`);
      await page.goto(AUDIENCES_URL, { waitUntil: 'networkidle', timeout: 60000 });
      await page.waitForTimeout(1200);
    } catch (err) {
      console.error(`[refresh] archive failed for "${nm}": ${err.message}`);
    }
  }
  return archived;
}
...
async function runRefresh({ trigger = 'manual' } = {}) {
  ...
  const newName = `${BASE_NAME} ${dateTag}`;
  const description =
    ...
    `via openai-audience-refresh. Uploaded ${dateTag}. Auto-refreshed monthly.`;
  ...
  console.log(`[refresh] ${hashes.length} hashes exported`);
  ...
  try {
    ...
    if (await isLoginPage(page)) {
      await notifyReauth(SEED_URL);
      await recordRun({ trigger, audience_name: newName, hash_count: hashes.length, status: 'expired' });
