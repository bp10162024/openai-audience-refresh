 ...
 * (the datacenter IP is what's tripping the bot-challenge). Set via env:
 *   PROXY_SERVER   e.g. "http://gate.example.com:7000"  (required to enable)
 *   PROXY_USERNAME / PROXY_PASSWORD  (if the proxy needs auth)
 * Returns undefined when PROXY_SERVER isn't set — Playwright then uses no proxy.
 */
